...
output_file="binsec_results_optimizations_clang_14.txt"
...
echo -e "Target\t\t\tO0\t\t\tO1\t\t\tO2\t\t\tO3\t\t\tOs\t\t\tOz\t\t\tOfast" > "$output_file"
...
for target in "${targets[@]}"; do
  ...
  for opt in "${optimizations[@]}"; do
    ...
    if grep -q "^starting from core" "$config_file"; then
      ...
      binsec_output=$(binsec -sse -sse-script "$config_file" -sse-depth "$depth" -checkct core_\$UNIQUE_BASE.snapshot -sse-timeout "$timeout")
    else
