...
output_file="binsec_results_optimizations_clang_19.txt"
...
for target in "${targets[@]}"; do
  ...
  for opt in "${optimizations[@]}"; do
    ...
    make CLANG="clang-14" "$target_with_opt"  # Build the target with the specific optimization level
    ...
    if grep -q "^starting from core" "$config_file"; then
      ...
      echo binsec -sse -sse-script "$config_file" -sse-depth "$depth" -checkct core_\$UNIQUE_BASE.snapshot
      binsec_output=$(binsec -sse -sse-script "$config_file" -sse-depth "$depth" -checkct core_\$UNIQUE_BASE.snapshot)
    else
