...
for PASS in "${OPT_PASSES[@]}"; do
    ...
    echo $CLANG -$OPT_LEVEL_CLANG -Xclang -disable-llvm-passes -fno-inline $CFLAGS -S -emit-llvm "$SOURCE_FILE" -o "$LLVM_IR"
    $CLANG -$OPT_LEVEL_CLANG -Xclang -disable-llvm-passes -fno-inline $CFLAGS -S -emit-llvm "$SOURCE_FILE" -o "$LLVM_IR"

    echo $OPT $NEW_PM -S "${ADDED_PASSES[@]}" "$LLVM_IR" -o "$OPTIMIZED_LL"
    $OPT $NEW_PM -S "${ADDED_PASSES[@]}" "$LLVM_IR" -o "$OPTIMIZED_LL"

    echo $CLANG -O0 $CFLAGS $OPTIMIZED_LL -o ${BASE_NAME}.out $LIBSYM
