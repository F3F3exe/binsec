...
for OPTIMIZATION in "${OPTIMIZATIONS[@]}"; do
  ...
  echo $CLANG $CFLAGS -$OPT_LEVEL -$OPTIMIZATION $SOURCE_FILE -o $BASE_NAME.out $LIBSYM $LIBHACL
  ERROR_OUTPUT=$($CLANG -w $CFLAGS -$OPT_LEVEL -$OPTIMIZATION $SOURCE_FILE -o $BASE_NAME.out $LIBSYM $LIBHACL  2>&1)
  echo $ERROR_OUTPUT
...
if grep -q "^starting from core" "$config_file"; then
    ...
    generate_combinations "${VALID_OPTIMIZATIONS[@]}" | parallel -j 28 "
        ...
        echo $CLANG $CFLAGS -$OPT_LEVEL {}  $SOURCE_FILE -o \$UNIQUE_BASE.out $LIBSYM $LIBHACL &&
        eval $CLANG $CFLAGS -$OPT_LEVEL {} $SOURCE_FILE -o \$UNIQUE_BASE.out $LIBSYM $LIBHACL &&
