...
for PASS in "${OPT_PASSES[@]}"; do
    ...
    #clang-14 -O1  -Xclang -disable-O0-optnone -m32 -march=i386 -DKRML_NOUINT128 -static -S -emit-llvm tls1_cbc_remove_padding_patch_wrapper.c -o tls1_cbc_remove_padding_patch.ll
    echo $CLANG -$OPT_LEVEL_CLANG -Xclang -disable-llvm-passes $CFLAGS -S -emit-llvm "$SOURCE_FILE" -o "$LLVM_IR"
    $CLANG -O1 -Xclang -disable-llvm-passes $CFLAGS -S -emit-llvm "$SOURCE_FILE" -o "$LLVM_IR"
